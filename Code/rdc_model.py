    ...
    if(isExist):
        ...
        for smx in range(len(result)):
            if(result[smx] > secMax and result[smx] < result[indexMax]):
                indexSecMax = smx
                secMax = result[smx]
        res1 = "respiratory disorder detected: {} with probability {:.0f}%".format(diseaseArray[indexMax], result[indexMax] * 100)
        res2 = "respiratory disorder detected: {} with probability {:.0f}%".format(diseaseArray[indexSecMax], result[indexSecMax] * 100)
